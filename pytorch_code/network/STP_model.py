...
class TemporalAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        sorted, indices = torch.sort(z, dim = 2, descending=True) #sort  
        for a in range(indices.size(0)):
            index = indices[a,:, 0:int(indices.size(2)/2), :,:]
            # print(index.shape)
            index = torch.squeeze(index)
            # print(index)
            output = out.index_select(2, index)
        return out, indices
...
class MaxPool2dSamePadding(nn.MaxPool2d):
    ...
    def forward(self, x):
        ...
        # out_t = np.ceil(float(t) / float(self.stride[0]))
        out_h = np.ceil(float(h) / float(self.stride[0]))
        out_w = np.ceil(float(w) / float(self.stride[1]))
        # pad_t = self.compute_pad(0, t)
...
class STP(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
        ...
        end_point = 'TA1'
        self.end_points[end_point] = TemporalAttn()
        if self._final_endpoint == end_point: return

        end_point = 'SA1'
        self.end_points[end_point] = SpatialAttn()
        if self._final_endpoint == end_point: return
